...
def get_profileInfo(vk_id, token=ACCESS_TOKEN):
    r = vk_api_request('users.get','user_id={}'.format(vk_id),token)
    return r
...
if __name__ == '__main__':
    ...
    parser.add_argument("vkids",nargs='+',help="Vk id or ids to add as root nodes")
    parser.add_argument("--jsonpath",type=str,help="Load graph from json")
    parser.add_argument("-k", "--keeproot",action="store_true",help="Keep the root node")
    ...
    if args.jsonpath is not None :
        ...
        for vid in args.vkids: 
            ...
            if args.keeproot:
                friends_network.remove_node(vid)
